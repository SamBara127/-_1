...
{
	std::cout << "Creating CircularBuffer...";
...
CircularBuffer::~CircularBuffer()
{
	std::cout << "Deleting CircularBuffer...";
...
{
	std::cout << "Copynig CircularBuffer...";
...
{
	std::cout << "Moving CircularBuffer...";
	main_buff.buffer = new value_type[cb.main_buff.capacity];
	main_buff.capacity = cb.main_buff.capacity;
	main_buff.size = cb.main_buff.size;
	main_buff.head = cb.main_buff.head;
	...
	cb.main_buff = {nullptr, 0, 0, 0, 0};
	std::cout << "     OK!\n";
}

CircularBuffer::CircularBuffer(int capacity)
{
	std::cout << "Creating CircularBuffer with capacity = " << capacity << " ...";
...
{
	std::cout << "Creating CircularBuffer with capacity = " << capacity << " and elements = '" << elem << "' ...";
	main_buff.buffer = new value_type[capacity];
	main_buff.capacity = capacity;
	main_buff.head = 0;
...
CircularBuffer& CircularBuffer::operator=(const CircularBuffer& cb)
{
    if (this != &cb)
    {
        ...
        main_buff.head = cb.main_buff.head;
        main_buff.tail = cb.main_buff.tail;
        std::copy(cb.main_buff.buffer, cb.main_buff.buffer + cb.main_buff.capacity, main_buff.buffer);
    }
    return *this;
}
